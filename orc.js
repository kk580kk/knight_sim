...
function orc(){
	...
	ev["orc_boss"]={
		ev:function(){
			...
			if (r == 1) {
				show("你潜入了兽人部落。")
				var ans = check("dex", orc_pow + 2 * getbuff("兽人的调教"))
			...
			if (r == 2) {
				...
				var x = gain({ e_exp: 2 })
				var ans = check("wis", orc_pow + 3 * x)
				if (ans >= 0) {
					show("你声称自己今晚原本要去给族长侍寝，却找不到族长的帐篷。")
					show("热心的卫兵替你指明了方向。")
				} else {
					show("你声称自己迷路了，找不到奴隶的住所。")
					show("卫兵立刻就发现了你话中的破绽——奴隶在晚上根本没时间休息。")
					gain({ v_exp: 5, p_exp: 3, s_exp: 2 }, "兽人卫兵")
					gainbuff("监禁：兽人", 1)
					return
				}
	...
	ev["orc_knight"] = {
		ev: function () {
			show("过去，你认为女骑士赢不了兽人这种说法是对骑士团的恶语中伤。")
			show("但现在你不得不信了。")
			show("畏惧降低了你对抗兽人的成功率。")
			orc_pow += 2
		},
		town: true,
		chance: function () {
			if ((status.name == "骑士" || status.name == "被诅咒的骑士") && getbuff("兽人的调教") >= 4) return 0.5
		}
	}
	ev["orc_prison"]={
		ev:function(){
...
function randomorcev(){
	...
	if (r == 3) {
		var rr=rand(4)
		if ((status.name == "骑士" || status.name == "被诅咒的骑士") && rand(2)==1) {
			show("在得知了你的骑士身份之后，兽人要求你在调教开始前先说一遍“唔，杀了我吧”。",true)
			show("然后……")
			show("“这种程度也配叫女骑士吗？再来！”")
			show("“给我叫得更有感情一点！”")
			show("“你分不清什么是感情，什么是色情吗？”")
			show("“拿出你的骑士精神来！”")
			show("你几乎产生了自己正在骑士团里接受训练的幻觉。",true)
			show("你久违地度过了一个没有被兽人侵犯的夜晚。")
			return
		}
		if (rr == 0) {
			show("兽人带你去看了一个做工粗糙的雕像。")
			show("你隐约能看出雕的是女人被兽人从后方侵犯。")
			show("兽人告诉你，这是兽人先祖和魔女。")
			show("魔女为了不被兽人侵犯，用魔法封印了自己的前穴。兽人就不停侵犯她的后穴，直到她自愿解咒。", true)
			...
			show("他一边向你讲述兽人征服人类王国的壮举，一边玩弄着你的身体。")
			gain({ v_exp: 2 }, "兽人", true)
		} else if (rr = 2) {
			show("兽人带你去看了一个做工粗糙的雕像。")
			...
			show("圣女的体液带有奇迹的力量，因此侵犯她的兽人永远不会感到疲惫。", true)
			show("他一边向你讲述兽人征服人类王国的壮举，一边玩弄着你的身体。")
			gain({ v_exp: 2 }, "兽人", true)
		}
	}
	if(r==4){
		if(getop("刺客")>=0 && rand(2)==0 && past_event.includes("orc_boss") && !("assassin_vs_orc"in flag)){
			...
			gain({ v_exp: 6, p_exp: 2, s_exp:2, e_exp: 2 }, "兽人族长")
